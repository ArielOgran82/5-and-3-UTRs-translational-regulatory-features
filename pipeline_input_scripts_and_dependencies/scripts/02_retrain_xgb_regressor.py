...
CORRECTED AGAIN to match the exact source notebook that produced the paper's
reported r=0.69 - traced to
~/.../notebooks_utr_ai_desktop/improved_cor._effic_utr_ai_main_IDO_version_24.12.25.ipynb
(NOT the "_2" version, which uses a different, later xgb_reg_tune variant
with symmetric scaled-y clipping instead of the asymmetric outlier trim
used here). Three things the previous attempt (r=0.59) was missing:
  1. 3'UTR features excluded entirely - both the regulatory numerics and
     the k-mer matrix are filtered to 5'UTR-only (regex drops any column
     matching 3'UTR-related patterns) before concatenation.
  2. Asymmetric outlier trim inside xgb_reg_tune: rows kept only where
     y is within (train_mean - 1*std, train_mean + 3*std), computed from
     train and applied to both train and test, BEFORE y-scaling (not the
     later notebook's symmetric bound on the scaled y).
  3. Split index 8 specifically (hardcoded `for n_split in [8]:` in the
     source cell, not a loop over all 10 splits) - not split 0.
"""
import pickle
import warnings
...
SPLIT = 8  # hardcoded in the source notebook cell: `for n_split in [8]:`
rnd_seed = RANDOM_SEEDS[SPLIT]
DROP_3UTR_PATTERN = r"(?:_3_|3$|3utr|utr3|mfe3|utr3_len)"
...
def xgb_reg_tune(X_train_, X_test_, y_train_num, y_test_num, n_estimators, learning_rate, max_depth, gamma):
    """Verbatim port of the source notebook's xgb_reg_tune: asymmetric
    outlier trim on raw y (train mean/std applied to both train and test),
    THEN scale y, THEN fit. Note this differs from the later "_2" notebook
    variant, which clips the already-scaled y symmetrically instead."""
    ...
    keep_train = (y_train_num > (avg_y_train - 1.0 * std_y_train)) & (y_train_num < (avg_y_train + 3.0 * std_y_train))
    y_train_num = y_train_num[keep_train]
    X_train_ = X_train_[keep_train, :]

    keep_test = (y_test_num > (avg_y_train - 1.0 * std_y_train)) & (y_test_num < (avg_y_train + 3.0 * std_y_train))
    y_test_num = y_test_num[keep_test]
...
def main():
    ...
    with open(f"{SCRATCH}/model2_xgb_regressor.pkl", "wb") as f:
        pickle.dump({
            ...
            "split_index": SPLIT, "random_seed": rnd_seed,
            "outlier_trim": "keep (train_mean - 1*std, train_mean + 3*std) on raw y, using train stats for both train and test",
            "utr_scope": "5'UTR-only (3'UTR-pattern columns dropped from both regulatory numerics and k-mer matrix)",
